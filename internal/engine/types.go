...
type Step struct {
	...
	Condition string                 `yaml:"condition,omitempty"` // Deprecated, use When
...
// SelectConfig defines interactive selection from step output
type SelectConfig struct {
	Prompt  string `yaml:"prompt"`
	From    string `yaml:"from"`    // Expression for array to select from
	Display string `yaml:"display"` // Template for display text
	Multi   bool   `yaml:"multi"`   // Allow multiple selection
}
